    ...
    def processCode(self):
        ...
        if len(origin) > 0:
            ...
            if origin in registrar:
                ...
                #for line in data:
                #    i += 1
               #     if (line.find("CSeq: 21 BYE")):
                #        data[i] = "CSeq: 21 AHOJ"

                #msg = data.count('CSeq: 21 BYE')
                #if (msg > 0):
                #    i = data.index('CSeq: 21 BYE')
                #    data[i] = "CSeq: 21 AHOJ"
                ir = -1
                index = -1
                for line in data:
                    index += 1
                    if line.find('Busy here') != -1:
                        ir = index
                        #data[index].replace('BYE', 'AHOJ')

                if (ir != -1):
                    data[ir] = data[ir].replace('Busy here', 'ZANEPRAZDNENY')


                #if ('CSeq: 21 BYE' in data):
                #    i = data.index('CSeq: 21 BYE')
                #    data[i] = "CSeq: 21 AHOJ"
                #    if (line =="CSeq: 21 BYE"):
                #print(data)
                #msg = data.index('CSeq: 21 BYE')
                #if (msg):
                #        line = "CSeq: 21 AHOJ"
                #print(data)
                text = "\r\n".join(data)
